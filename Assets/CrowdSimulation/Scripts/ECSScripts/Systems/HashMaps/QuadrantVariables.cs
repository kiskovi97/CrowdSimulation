...
namespace Assets.CrowdSimulation.Scripts.ECSScripts.Systems
{
    public class QuadrantVariables
    {
        ...
        public static KeyDistance IndexFromPosition(float3 realWorldPosition, float3 prev, MapValues max)
        {
            ...
            var rounded = math.round(indexPosition);
            ...
            {
                key = Index(i, j, max),
                distance = math.length(ConvertToLocal(prev, max) - rounded),
                roundedPosition = ConvertToWorld(rounded, max),
            };
        }
        ...
        {
            return (realWorldPosition - max.offset + new float3(max.maxWidth, 0, max.maxHeight)) * StaticMap.density;
        }

        public static float3 ConvertToWorld(float3 position, MapValues max)
        {
            return position * (1f / StaticMap.density) - new float3(max.maxWidth, 0, max.maxHeight) + max.offset;
        }
    }
}
